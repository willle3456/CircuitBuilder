...
class FFNN(object):
    def __init__(self, layers, activation=tf.tanh, output_activation=tf.identity, post_function=tf.identity, input_vector=None, session=None):
        ...
        for i in range(len(self.layers) - 1):
            self.weights.append(tf.Variable(tf.random_uniform(
                [self.layers[i], self.layers[i + 1]], -1, 1)))
            self.biases.append(tf.Variable(
                tf.random_uniform([self.layers[i + 1]], -1, 1)))
        ...
        self.output = self.output_activation(
            tf.matmul(self.output, self.weights[-1]) + self.biases[-1])
        ...
        if session is not None:
            #self.session = tf.InteractiveSession()
            self.saver.restore(
                self.session, "/home/ssmarok/Documents/SBHacks/CircuitBuilder/FeedForwardNeuralNet/session/model.ckpt")
            print("Session Restored")
        #else:
            #self.session = session

        # Save path
        #self.save_path = saver.save(sess, 'my_test_model.ckpt')
        #self.save_path = saver.save(
        #    self.session, "~/Documents/SBHacks/CircuitBuilder/FeedForwardNeuralNet/session/model.ckpt")
    ...
    def train(self, train_in, train_out, loss_func=None, optimizer=None, batch_size=10, epochs=1, lc_interval=10):
        ...
        for i in range(epochs):
            ...
            self.session.run(train_step, feed_dict={
                             self.input: in_batch, self.train_targets: out_batch})

            if self.epochs % lc_interval == 0:
                self.learning_curve.append((
                    self.epochs,
                    self.session.run(loss_val, feed_dict={
                                     self.input: train_in, self.train_targets: train_out})
                ))
                #print("Reached epoch {}".format(self.epochs))
            self.epochs += 1
            self.epochs_global = self.session.run(
    ...
    def save_session(self):
        #self.save_path = self.saver.save(self.session, 'test_model.ckpt')
